    ...
    for i in range(3):
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"⏳ Model loading... waiting 20s (Attempt {i+1}/3)")
                time.sleep(20)
            else:
...
def send_intel_email(report_path, map_path):
    if not SENDER_EMAIL or not GMAIL_PASSWORD:
        print("⚠️ Email secrets missing. Skipping dispatch.")
        return

    print("📧 Dispatching Intelligence Brief to Inbox...")
    msg = MIMEMultipart()
    msg['From'] = SENDER_EMAIL
    msg['To'] = RECIPIENT_EMAIL
    msg['Subject'] = f"🚨 SATARK INTEL: {datetime.now().strftime('%B %Y')}"
...
def run_monthly_audit():
    print("🚀 SATARK V8.0 (No-Billing Edition): STARTING...")
